...
public class MockingHandler : DelegatingHandler
{
	...
	private static HttpResponseMessage BuildResponse(string host)
	{
		...
		if (string.Equals(host, "download.kiwix.org", StringComparison.OrdinalIgnoreCase))
		{
			var path = Path.Combine(".", "Data", "library_zim.xml");
			var stream = File.Open(path, FileMode.Open, FileAccess.Read, FileShare.Read);
			return new() { Content = new StreamContent(stream), };
		}

		if (string.Equals(host, "lbo.download.kiwix.org", StringComparison.OrdinalIgnoreCase))
		{
